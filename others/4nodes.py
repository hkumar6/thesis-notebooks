...
graph_list4 = pickle.load(
    open("/Users/kumarharsha/thesis/graph_data/digraph_ksort_4.pkl", "rb"))

all_graphs = []
for k in graph_list4.keys():
        all_graphs = all_graphs + graph_list4[k]

num_rows = len(all_graphs)

p = nx.circular_layout(all_graphs[0]['digraph'])

#fig, axes = plt.subplots(num_rows,9, num=1, figsize=(35,36))
fig = plt.figure(figsize=(30,100))
i = 1
for g in all_graphs:
    # directed graph
    ax = fig.add_subplot(num_rows, 10, i)
    ax.axis('off')
    if(i == 1):
    ...
    # scores
    l = net_sym.out_degree_laplacian(g['digraph'], node_list)
    u, s, v = np.linalg.svd(l)
    score = u.dot(np.diag(s))
    ax = fig.add_subplot(num_rows, 10, i+2, projection='3d')
    ...
    if(q.shape[0] == 3):
        ax = fig.add_subplot(num_rows, 10, i+3, projection='3d')
        all_vectors = np.array([
        ...
        ax = fig.add_subplot(num_rows, 10, i+3)
        if(q.shape[0] == 2):
            ax.quiver(*origin,
                      np.asarray(q[0,:])[0],
                      np.asarray(q[1,:])[0],
    ...
    # symmetrised graph
    eqG = g['sym']
    ax = fig.add_subplot(num_rows, 10, i+4)
    ax.axis('off')
    if(i == 1):
    ...
    ax = fig.add_subplot(num_rows, 10, i+5)
    ax.axis('off')
    if(i == 1):
    ...
    # reduced laplacian
    ax = fig.add_subplot(num_rows, 10, i+6)
    ax.axis('off')
    if(i == 1):
    ...
    # sigma
    ax = fig.add_subplot(num_rows, 10, i+7)
    ax.axis('off')
    if(i == 1):
    ...
    # X
    ax = fig.add_subplot(num_rows, 10, i+8)
    ax.axis('off')
    if(i == 1):
    ...
    ax = fig.add_subplot(num_rows, 10, i+9)
    ax.axis('off')
    if(i == 1):
